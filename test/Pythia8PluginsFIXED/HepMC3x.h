...
/*
    for (int i = 1; i < pyev.size(); ++i) {
      ...
      GenVertexPtr end_vtx = hepevt_particles[i]->end_vertex();
         for (unsigned int j = 0; j < daughters.size(); ++j)
            if (hepevt_particles[daughters[j]]->production_vertex()) end_vtx = hepevt_particles[daughters[j]]->production_vertex();
      
      if (!end_vtx) {
          end_vtx = make_shared<GenVertex>();
          nvertices++;          
      }
      end_vtx->add_particle_in(hepevt_particles[i]);
      for (unsigned int j = 0; j < daughters.size(); ++j) end_vtx->add_particle_out( hepevt_particles[daughters[j]] );
    }
*/

    // Reserve memory for the event.
    evt->reserve( hepevt_particles.size(), nvertices );

    // Here we assume that the first two particles are the beam particles.
    vector<GenParticlePtr> beam_particles;
    for (auto& p: hepevt_particles) if (!p->production_vertex()) beam_particles.push_back(p);
